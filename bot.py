...
async def gerar_audio(texto):
    try:
        ...
        if not ELEVENLABS_API_KEY or not ELEVENLABS_VOICE_ID:
            print("ERRO ELEVENLABS: Variáveis faltando no arquivo .env")
            return None
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data, headers=headers, timeout=30) as r:
                ...
                print(f"ERRO ELEVENLABS (Status {r.status}): {await r.text()}")
                return None
        ...
        if texto.startswith("evac/"):
            ...
            if audio_bytes_io:
                ...
                try:
                    ...
                    await message.reply(
                        f"{resposta}\n\n⚠️ *(Erro ao processar áudio local: {e})*"
                    )
                finally:
                ...
                aviso_erro = "A ElevenLabs rejeitou a requisição. Motivos prováveis: Seus caracteres gratuitos acabaram (limite de 10k/mês) ou a sua API Key está inválida."
                if not ELEVENLABS_API_KEY:
                    aviso_erro = "A chave `ELEVENLABS_API_KEY` está vazia ou ausente no seu arquivo `.env`."
                elif not ELEVENLABS_VOICE_ID:
                    aviso_erro = "O ID da voz `ELEVENLABS_VOICE_ID` está vazio ou ausente no seu arquivo `.env`."

                await message.reply(f"{resposta}\n\n❌ **[Erro de Voz]:** {aviso_erro}")
